...
import { getMedicalHistories } from 'apiSdk/medical-histories';
...
function DashboardPage() {
  ...
  const medicalHistoryStat = useSWR<PaginatedInterface<MedicalHistoryInterface>>(
    () => `/medical-histories`,
    () => getMedicalHistories({ limit: 1, offset: 0 }),
  );
  ...
  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    ...
    },
    {
      ...
      result: medicalHistoryStat?.data?.totalCount,
      isLoading: medicalHistoryStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Schedules',
